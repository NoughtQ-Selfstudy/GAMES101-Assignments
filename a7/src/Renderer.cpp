...
void Renderer::Render(const Scene& scene)
{
    ...
    // Bonus: Speeding up by multithreading
    int num_threads = std::max(1, (int)std::thread::hardware_concurrency() - 2);
    std::vector<std::thread> threads;
    std::atomic<int> progress(0);

    auto render_row = [&](int start_row, int end_row) {
    ...
    for (int i = 0; i < num_threads; ++i) {
        ...
        int end = (i + 1) * scene.height / num_threads;
        threads.emplace_back(render_row, start, end);
    }
